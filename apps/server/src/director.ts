...
type DirectorAction = DispatchAction | CiGateAction | DecomposeAction | DoneAction | GiveUpAction;
...
2. Reflect. Each turn, look at the last 1-2 outcomes. Do not repeat what just failed identically.
3. Hard limits: do not dispatch the same sub-agent more than 4 times in one run. After 3 cycles without progress, request_decompose or give_up.
4. Always run ci_gate before mark_done. If ci_gate fails twice for the same root cause, give_up — do not loop forever.
5. request_decompose when the ticket spans unrelated concerns (infra + code + docs as independent threads). CTO will create subtickets and end this run cleanly.
6. Notes are CONCRETE: "Add /version endpoint to api/ following HealthController pattern, smoke test required" — NOT "do the ticket".
