...
export function defaultWorkflowForProject(projectId: string): WorkflowDefinition {
  ...
  const junior = byName.get("PHP Junior Coder") ?? byName.get("Junior Coder") ?? byName.get("Coder") ?? firstByRole.get("coder");
  const senior = byName.get("PHP Senior Coder") ?? byName.get("Senior Coder");
